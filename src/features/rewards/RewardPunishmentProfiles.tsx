...
const actionDefinitions =
  rewardPunishmentActions as readonly RewardPunishmentActionDefinition[];
    ...
    return rewardPunishmentCategories
      .map((category) => ({
        ...
        items: visiblePrimitives.filter(
          (primitive) => primaryContextCategoryId(primitive) === category.id,
        ),
      }))
    ...
    <section className="rp-stack">
      <article className="rp-heading panel">
        <div>
          ...
          <p>
            Reward and punishment are separate contextual uses, not opposite ends of one
            scale. Something can work strongly as both, one, or neither without changing
            your general kink preference.
          </p>
        </div>
        ...
        <>
          <div className="rp-tabs" role="group" aria-label="Context profile">
            {(["reward", "punishment"] as const).map((target) => (
              <button
                type="button"
                key={target}
                className={context === target ? "rp-tab is-active" : "rp-tab"}
          ...
          <section className="rp-summary panel">
            <div>
              ...
              <h2>
                {activeCounts.direct} directly rated · {activeCounts.random} in random
                pool
              </h2>
            </div>
            <p>
          ...
          <section className="rp-patterns panel">
            <div className="rp-patterns-heading">
              ...
              <p>
                Affinity and evidence breadth are separate. One strong answer can start a
                pattern, but it cannot fake the confidence of a well-explored category.
              </p>
            </div>

            {categoryProfile.length === 0 ? (
              <div className="rp-patterns-empty">
                Rate a few {contextNoun(context)} items and category patterns will start
                appearing here.
              </div>
            ) : (
          ...
          <section className="rp-suggestions panel">
            <div className="rp-suggestions-heading">
              ...
              <p>
                ...
                conservative <strong>Works</strong> state and does not add it to the random
                pool.
              </p>
            </div>
          ...
          <div className="rp-toolbar panel">
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) =>
                  setSuitabilityFilter(event.target.value as SuitabilityFilter)
                }
              >
          ...
          <ExpandableGroupedList
            ...
            renderItem={(primitive) => {
              ...
              return (
                <article
                  className={
                    state.suitability === "never" ? "rp-row is-never" : "rp-row"
                  }
                ...
                >
                  <div className="rp-row-main">
                    <div className="rp-item-title">
                      ...
                      <span className="rp-source-badge">
                        {primitive.sourceType === "catalog" ? "Catalog" : "Action"}
                      </span>
                    </div>
                    ...
                    {generalPreference && (
                      <div className="rp-general-hint">
                        General preference: {catalogPreferenceLabels[generalPreference]}
                      </div>
                    )}
                    {proposal && (
                  ...
                  <label className="rp-suitability-editor">
                    <span className="sr-only">
                      {contextTitle(context)} suitability for {primitive.label}
                    </span>
                    <select
                          ...
                          <dd>
                            {primitive.sourceOrigins.length > 0
                              ? primitive.sourceOrigins
                                  .map(
                                    (origin) =>
                                      origin.sourceSheet + " · row " + origin.sourceRow,
                                  )
                                  .join(" · ")
