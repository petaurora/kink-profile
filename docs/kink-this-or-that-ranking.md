...
Temporal reranking is scoped separately in [M11 This-or-That Ranking History & Movement](m11-ranking-history-movement.md). M11 preserves each ranking run as history instead of treating a fresh rerank as destructive deletion or as lifetime score accumulation.
...
M11 adds a separate temporal concept: **Start a new ranking run**. That action archives the current run and begins fresh comparative scoring while preserving history. It must not be conflated with destructive reset controls.
