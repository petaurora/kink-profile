...
6. [M6 Catalog Integration](m6-catalog-integration.md)  
   The in-progress M6 contract: C1 stable IDs, C2 metadata/signal mappings, and C3 explicit preference are implemented; C4 ranking hardening is next, followed by catalog result integration, affinity foundation, and the M7 boundary.

7. [M6 C3 Explicit Preference](m6-c3-explicit-preference.md)  
   The implemented C3 contract: state semantics, catalog-profile storage, legacy ranking migration, eligibility, comparison-card editing, tests, and acceptance scenarios.
