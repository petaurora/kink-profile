...
M11 extends the existing This-or-That ranking system with temporal history.

The existing ranking engine answers:

> "How do these interests rank relative to one another?"

M11 adds a second question:
...
M11 must never overwrite or reinterpret them.
...
M11 therefore treats repeated This-or-That sessions as **separate temporal runs**, not as one endlessly accumulating lifetime score.
...
M11 must migrate that data without loss.
...
M11 changes the temporal meaning of the pairwise source but not the source-aware architecture.
...
M11 v1 does **not** need a large analytics dashboard.
...
M9 full-profile export should include, once M11 exists:
...
to the default share artifact merely because M11 stores them.
...
## M11.1 — Run-aware persistence + migration
...
## M11.3 — Previous-rank movement indicators
...
## M11.4 — Profile/lifecycle integration
